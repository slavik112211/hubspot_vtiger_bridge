...
response = JSON.parse(Net::HTTP.get(URI.parse(VTIGER_ADDRESS+'webservice.php?operation=describe&elementType=Leads&sessionName=' + session_id)))
response = JSON.pretty_generate(response)
File.open('leadStructureVtiger.js', 'w') {|f| f.write(response) }